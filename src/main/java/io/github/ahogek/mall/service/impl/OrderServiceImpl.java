...
public class OrderServiceImpl implements IOrderService {
    ...
    public ResponseVo<OrderVo> create(Integer uid, Integer shippingId) {
        ...
        for (Cart cart : cartList) {
            ...
            if (!ProductStatusEnum.ON_SALE.getCode().equals(product.getStatus())) {
                return ResponseVo.error(ResponseEnum.PRODUCT_OFF_SALE_OR_DELETE,
                        ResponseEnum.PRODUCT_OFF_SALE_OR_DELETE.getDesc() + " " + product.getName());
            }
            // 库存是否充足
        ...
        // 计算总价，只计算选中的商品
        // 生成订单，入库：order 和 order_item,事务控制同时成功或都失败
        ...
        // 更新购物车（选中的商品）
        // Redis have Transaction (package command) but can't rollback.
        for (Cart cart : cartList) {
    ...
    public ResponseVo<PageInfo> list(Integer uid, Integer pageNum, Integer pageSize) {
        ...
        Map<Long, List<OrderItem>> orderItemMapByOrderNo = orderItemList.stream()
                .collect(Collectors.groupingBy(OrderItem::getOrderNo));
        Set<Integer> shippingIdSet = orderList.stream().map(Order::getShippingId).collect(Collectors.toSet());
        List<Shipping> shippingList = shippingMapper.selectByIdSet(shippingIdSet);
        Map<Integer, Shipping> shippingMapById = shippingList.stream().collect(Collectors
                .toMap(Shipping::getId, shipping -> shipping));
        List<OrderVo> orderVoList = new ArrayList<>();
        for (Order order : orderList) {
            OrderVo orderVo = buildOrderVo(order, orderItemMapByOrderNo.get(order.getOrderNo()),
                    shippingMapById.get(order.getShippingId()));
    ...
    public ResponseVo<OrderVo> detail(Integer uid, Long orderNo) {
        ...
        if (order == null || !Objects.equals(uid, order.getUserId())) {
    ...
    @Override
    public ResponseVo<String> cancel(Integer uid, Long orderNo) {
        Order order = orderMapper.selectByOrderNo(orderNo);
        if (order == null || !Objects.equals(uid, order.getUserId())) {
            return ResponseVo.error(ResponseEnum.ORDER_NOT_EXIST);
        }
